...
const members = [
  ...
  { id: 4, name: 'Rev. Dr. Adesina Abegunde', position: 'Head of Pastors', img: pastor4 }, // Giant one
      ...
      <div className="row justify-content-center align-items-end text-center">
        {members.map((member, index) => {
          ...
          return (
            <div
              key={member.id}
              className={`mb-4 ${isHead ? 'col-md-4 col-lg-6' : 'col-1'}`}
            >
              <div className={`card h-100 border-${isHead ? 'primary' : 'secondary'}  shadow`} style={{ width: isHead ? '100%' : '145px' }}>
                <img
                  src={member.img}
                  alt={member.name}
                  className={`mx-auto card-top d-block mt-3 ${isHead ? 'giant-img' : 'member-img'}`}
                />
